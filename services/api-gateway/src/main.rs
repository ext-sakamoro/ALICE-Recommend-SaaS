...
async fn health(State(state): State<AppState>) -> Json<HealthResponse> {
    Json(HealthResponse {
        status: "ok",
        service: "alice-recommend-saas-gateway",
...
async fn main() {
    ...
    let upstream = std::env::var("UPSTREAM_URL")
        .unwrap_or_else(|_| "http://localhost:8122".to_string());

    let routes: DashMap<String, String> = DashMap::new();
    routes.insert("/api/v1/recommend".to_string(), upstream.clone());
    ...
    tracing::info!("alice-recommend-saas-gateway listening on {addr}");
